/*
 * Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
 */

import * as z from "zod";
import { remap as remap$ } from "../lib/primitives.js";
import { safeParse } from "../lib/schemas.js";
import { Result as SafeParseResult } from "../types/fp.js";
import {
  AddressStatus,
  AddressStatus$inboundSchema,
  AddressStatus$outboundSchema,
} from "./addressstatus.js";
import {
  CountryCodeEnum,
  CountryCodeEnum$inboundSchema,
  CountryCodeEnum$outboundSchema,
} from "./countrycodeenum.js";
import { SDKValidationError } from "./errors/sdkvalidationerror.js";
import {
  SourceEnum,
  SourceEnum$inboundSchema,
  SourceEnum$outboundSchema,
} from "./sourceenum.js";

export type TransactionImportCustomer = {
  /**
   * Phone number associated with the address.
   */
  phone?: string | null | undefined;
  /**
   * Primary street address.
   */
  street1?: string | null | undefined;
  /**
   * Additional street address details, such as an apartment or suite number.
   */
  street2?: string | null | undefined;
  /**
   * City where the customer resides.
   */
  city?: string | null | undefined;
  /**
   * County or district of the customer.
   */
  county?: string | null | undefined;
  /**
   * State or province of the customer.
   */
  state?: string | null | undefined;
  /**
   * ZIP or Postal code of the customer.
   */
  postalCode?: string | null | undefined;
  /**
   * Country code in ISO 3166-1 alpha-2 format
   */
  country?: CountryCodeEnum | null | undefined;
  /**
   * Complete address string of the customer, which can be used as an alternative to individual fields.
   */
  fullAddress?: string | null | undefined;
  externalId?: string | null | undefined;
  name?: string | null | undefined;
  email?: string | null | undefined;
  organizationId?: string | null | undefined;
  connectionId?: string | null | undefined;
  source?: SourceEnum | undefined;
  registrationNumber?: string | null | undefined;
  addressStatus?: AddressStatus | undefined;
};

/** @internal */
export const TransactionImportCustomer$inboundSchema: z.ZodType<
  TransactionImportCustomer,
  z.ZodTypeDef,
  unknown
> = z.object({
  phone: z.nullable(z.string()).optional(),
  street_1: z.nullable(z.string()).optional(),
  street_2: z.nullable(z.string()).optional(),
  city: z.nullable(z.string()).optional(),
  county: z.nullable(z.string()).optional(),
  state: z.nullable(z.string()).optional(),
  postal_code: z.nullable(z.string()).optional(),
  country: z.nullable(CountryCodeEnum$inboundSchema).optional(),
  full_address: z.nullable(z.string()).optional(),
  external_id: z.nullable(z.string()).optional(),
  name: z.nullable(z.string()).optional(),
  email: z.nullable(z.string()).optional(),
  organization_id: z.nullable(z.string()).optional(),
  connection_id: z.nullable(z.string()).optional(),
  source: SourceEnum$inboundSchema.optional(),
  registration_number: z.nullable(z.string()).optional(),
  address_status: AddressStatus$inboundSchema.optional(),
}).transform((v) => {
  return remap$(v, {
    "street_1": "street1",
    "street_2": "street2",
    "postal_code": "postalCode",
    "full_address": "fullAddress",
    "external_id": "externalId",
    "organization_id": "organizationId",
    "connection_id": "connectionId",
    "registration_number": "registrationNumber",
    "address_status": "addressStatus",
  });
});

/** @internal */
export type TransactionImportCustomer$Outbound = {
  phone?: string | null | undefined;
  street_1?: string | null | undefined;
  street_2?: string | null | undefined;
  city?: string | null | undefined;
  county?: string | null | undefined;
  state?: string | null | undefined;
  postal_code?: string | null | undefined;
  country?: string | null | undefined;
  full_address?: string | null | undefined;
  external_id?: string | null | undefined;
  name?: string | null | undefined;
  email?: string | null | undefined;
  organization_id?: string | null | undefined;
  connection_id?: string | null | undefined;
  source?: string | undefined;
  registration_number?: string | null | undefined;
  address_status?: string | undefined;
};

/** @internal */
export const TransactionImportCustomer$outboundSchema: z.ZodType<
  TransactionImportCustomer$Outbound,
  z.ZodTypeDef,
  TransactionImportCustomer
> = z.object({
  phone: z.nullable(z.string()).optional(),
  street1: z.nullable(z.string()).optional(),
  street2: z.nullable(z.string()).optional(),
  city: z.nullable(z.string()).optional(),
  county: z.nullable(z.string()).optional(),
  state: z.nullable(z.string()).optional(),
  postalCode: z.nullable(z.string()).optional(),
  country: z.nullable(CountryCodeEnum$outboundSchema).optional(),
  fullAddress: z.nullable(z.string()).optional(),
  externalId: z.nullable(z.string()).optional(),
  name: z.nullable(z.string()).optional(),
  email: z.nullable(z.string()).optional(),
  organizationId: z.nullable(z.string()).optional(),
  connectionId: z.nullable(z.string()).optional(),
  source: SourceEnum$outboundSchema.optional(),
  registrationNumber: z.nullable(z.string()).optional(),
  addressStatus: AddressStatus$outboundSchema.optional(),
}).transform((v) => {
  return remap$(v, {
    street1: "street_1",
    street2: "street_2",
    postalCode: "postal_code",
    fullAddress: "full_address",
    externalId: "external_id",
    organizationId: "organization_id",
    connectionId: "connection_id",
    registrationNumber: "registration_number",
    addressStatus: "address_status",
  });
});

/**
 * @internal
 * @deprecated This namespace will be removed in future versions. Use schemas and types that are exported directly from this module.
 */
export namespace TransactionImportCustomer$ {
  /** @deprecated use `TransactionImportCustomer$inboundSchema` instead. */
  export const inboundSchema = TransactionImportCustomer$inboundSchema;
  /** @deprecated use `TransactionImportCustomer$outboundSchema` instead. */
  export const outboundSchema = TransactionImportCustomer$outboundSchema;
  /** @deprecated use `TransactionImportCustomer$Outbound` instead. */
  export type Outbound = TransactionImportCustomer$Outbound;
}

export function transactionImportCustomerToJSON(
  transactionImportCustomer: TransactionImportCustomer,
): string {
  return JSON.stringify(
    TransactionImportCustomer$outboundSchema.parse(transactionImportCustomer),
  );
}

export function transactionImportCustomerFromJSON(
  jsonString: string,
): SafeParseResult<TransactionImportCustomer, SDKValidationError> {
  return safeParse(
    jsonString,
    (x) => TransactionImportCustomer$inboundSchema.parse(JSON.parse(x)),
    `Failed to parse 'TransactionImportCustomer' from JSON`,
  );
}
