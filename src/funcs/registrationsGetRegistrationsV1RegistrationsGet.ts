 ...
 *     jurisdictions.
 */
export function registrationsGetRegistrationsV1RegistrationsGet(
  client: SDKCore,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.GetRegistrationsV1RegistrationsGetRequest$outboundSchema
  ...
  const path = pathToFunc("/v1/registrations")();

  const query = encodeFormQuery({
    "country_code__in": payload?.country_code__in,
    "filing_frequency__in": payload?.filing_frequency__in,
  ...
  const context = {
    ...
    operationID: "get_registrations_v1_registrations_get",
    oAuth2Scopes: [],
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, models.PageRegistrationReadWithPassword$inboundSchema),
